...
Cet atelier est noté sur **20 points** selon le barème suivant :

### ✅ 1. Repository exécutable sans erreur (4/20)

**Critères :**
- Une seule commande : `make all`
...
**Validation :**
```bash
make all  # Doit s'exécuter sans erreur
make status  # Affiche l'état complet
```
...
- L'application web s'affiche correctement
...
- ✅ **Scripts auxiliaires** (check-deps.sh, import-image.sh)
- ✅ **Pipeline one-command** : `make all`
- ✅ **Gestion d'erreurs** dans les scripts
- ✅ **Idempotence** : relançable sans problème
- ✅ **Help intégré** : `make help`

**Commandes disponibles :**
```bash
make help           # Liste toutes les commandes
make all            # Pipeline complet
make check-deps     # Vérification automatique
make install-deps   # Installation automatique
make clean          # Nettoyage automatique
```
...
- ✅ Guide pas à pas pour chaque séquence
...
```bash
# Commits réguliers et bien nommés
git commit -m "feat: Ajout du template Packer pour Nginx"
git commit -m "feat: Configuration Ansible pour déploiement K3d"
git commit -m "feat: Ajout Makefile avec pipeline complet"
git commit -m "docs: Documentation README complète"
git commit -m "fix: Correction syntaxe Ansible"

# Structure modulaire
- Dossiers bien organisés (packer/, ansible/, k8s/, scripts/)
...
- Messages d'aide dans les scripts
```
...
Ce projet démontre une maîtrise complète du cycle DevOps moderne :

1. ✅ **Build** : Création d'artefacts reproductibles avec Packer
2. ✅ **Store** : Gestion d'images Docker
3. ✅ **Deploy** : Orchestration avec Ansible
4. ✅ **Run** : Exécution sur Kubernetes (K3d)
5. ✅ **Monitor** : Vérification de l'état du cluster

**Cette approche Infrastructure as Code garantit :**
- **Reproductibilité** : Même résultat sur n'importe quel environnement
- **Traçabilité** : Historique complet dans Git
- **Scalabilité** : Facilement adaptable à d'autres applications
...
*Document généré le 05 février 2026 - Yilizire - EFREI Paris M2*
